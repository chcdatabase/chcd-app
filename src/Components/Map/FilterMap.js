...
const loadPAffIndex = ( inputValuePAff: string, callback: (options: props.pAffIndex) => void) => {
  setTimeout(() => {callback(props.fetchPAffIndex(inputValuePAff));}, 1000);
};
const loadAffIndex = ( inputValueAff: string, callback: (options: props.affIndex) => void) => {
  setTimeout(() => {callback(props.fetchAffIndex(inputValueAff));}, 1000);
};

const loadNatIndex = ( inputValueNat: string, callback: (options: props.natIndex) => void) => {
  setTimeout(() => {callback(props.fetchNatIndex(inputValueNat));}, 1000);
};
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i][0] === props.institution_category){
      ...
      for (let t = 0; t < data2.length; t++) {
        let i = data2[t];
        subcat.push(<option value={i}>{cat_trans[0][i.replace(/\s+$/, '').replace(/\s|\//g, '_').toLowerCase()][props.language]}</option>)
      }
    }
    else {}
  }
  ...
  for (let i = 0; i < dataEvents.length; i++) {
    if (dataEvents[i][0] === props.event_category){
      ...
      for (let t = 0; t < dataEvents2.length; t++) {
        let i = dataEvents2[t];
        subcatEvents.push(<option value={i}>{cat_trans[0][i.replace(/\s+$/, '').replace(/\s|\//g, '_').toLowerCase()][props.language]}</option>)
      }
    }
    else {}
  }
      ...
      <div className={props.filterDisplay}>
        ...
        <Form.Group className="mb-1"><Row><Col>
            ...
            <Form.Select
              ...
              <option value="Events">{translate[0]["events"][props.language]}</option>
            </Form.Select>
        </Col></Row></Form.Group>
        ...
        <div className={props.filterDisplay}>
          ...
          <Form.Group className="mb-1"><Row><Col>
              ...
              <Select
                ...
                getOptionLabel={option => family_trans[0][option.value.replace(/\s+$/, '').replace(/\s|\//g, '_').toLowerCase()][props.language] }
                onChange={(e) =>  props.handleChangeData(e)}
                components={{ IndicatorSeparator:() => null }}
              />
          </Col></Row></Form.Group>
          ...
          <Form.Group className="mb-1"><Row><Col>
              ...
              <Form.Select
                ...
                {props.instCatsIndex.map((node, i) => {
                  let val = node[0];
                  return (<option data-index={i} value={val}>{cat_trans[0][val.replace(/\s+$/, '').replace(/\s|\//g, '_').toLowerCase()][props.language]}</option>)
                })}
              </Form.Select>
...
// EVENT FORM CONSTRUCTOR ////////////////////////////////////////////////////////////////////////////////
      ...
      return (
        ...
        <div className={props.filterDisplay}>
          ...
          {/* NAME SELECT ////////////////////////////////////////////////////////////////// */}
          <Form.Group className="mb-1"><Row><Col>
            <Form.Label className="filter_label mb-0" data-tip data-for="inst_name">{translate[0]["event_name"][props.language]}</Form.Label>
              <ReactTooltip id="event_name" place="right" effect="solid">{translate[0]["enter_name"][props.language]}</ReactTooltip>
            <Form.Control type="text"
              name="name_western"
          ...
          <Form.Group className="mb-1"><Row><Col>
              <Form.Label className="filter_label mb-0" data-tip data-for="event_category">{translate[0]["event_category"][props.language]}</Form.Label>
                <ReactTooltip id="event_category" place="right" effect="solid">{translate[0]["select_start"][props.language]} {translate[0]["category"][props.language]}</ReactTooltip>
              <Form.Select
                name="event_category"
                aria-label={translate[0]["event_category"][props.language]}
          ...
          <Form.Group className="mb-1"><Row><Col>
              <Form.Label className="filter_label mb-0" data-tip data-for="event_subcategory">{translate[0]["event_subcategory"][props.language]}</Form.Label>
                <ReactTooltip id="event_subcategory" place="right" effect="solid">{translate[0]["select_start"][props.language]} {translate[0]["subcategory"][props.language]}</ReactTooltip>
              <Form.Select
                name="event_subcategory"
                aria-label={translate[0]["event_subcategory"][props.language]}
